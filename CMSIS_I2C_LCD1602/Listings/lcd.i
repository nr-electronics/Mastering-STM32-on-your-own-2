...
static __inline void DelayMicro(volatile uint32_t micros)
{
	micros *=(SystemCoreClock / 1000000) / 9;
	while (micros--);
}

static __inline void DelayNano(volatile uint32_t nanos)
{
...
{
  I2C_SendByteByADDR(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x5400)), bt,0x4E);
}
...
{
	if(mode==0) LCD_WriteByteI2CLCD(portlcd&=~0x01);
	else LCD_WriteByteI2CLCD(portlcd|=0x01);
	uint8_t hc=0;
	hc=c>>4;
...
void LCD_ini(void)
{
  ...
  LCD_WriteByteI2CLCD(portlcd&=~0x02);
  delay_ms(100);
  sendhalfbyte(0x03);
  DelayMicro(4500);
  sendhalfbyte(0x03);
  ...
  sendbyte(0x02,0);
  delay_ms(2);
  LCD_WriteByteI2CLCD(portlcd|=0x08);
}
